...
def _get_owned_session(db: Session, org_id: int, session_id: str) -> ChatSession:
    session = (
        db.query(ChatSession)
        .filter(ChatSession.id == session_id, ChatSession.org_id == org_id)
        .first()
    )
    if not session:
...
def create_session(db: Session, org_id: int, title: str | None = None) -> dict:
    session = ChatSession(id=str(uuid.uuid4()), org_id=org_id, title=title)
    db.add(session)
    db.commit()
    db.refresh(session)
...
def list_sessions(db: Session, org_id: int, page: int = 1, page_size: int = 20) -> dict:
    safe_page = max(int(page or 1), 1)
    safe_page_size = min(max(int(page_size or 20), 1), 100)

    query = db.query(ChatSession).filter(ChatSession.org_id == org_id)
...
def get_session(db: Session, org_id: int, session_id: str) -> dict:
    return _session_to_dict(_get_owned_session(db, org_id, session_id))


def get_session_messages(db: Session, org_id: int, session_id: str, page: int = 1, page_size: int = 50) -> dict:
    _get_owned_session(db, org_id, session_id)

    safe_page = max(int(page or 1), 1)
    safe_page_size = min(max(int(page_size or 50), 1), 200)

    query = db.query(ChatMessage).filter(ChatMessage.session_id == session_id, ChatMessage.org_id == org_id)
    total_count = query.count()
    messages = (
...
def delete_session(db: Session, org_id: int, session_id: str) -> None:
    session = _get_owned_session(db, org_id, session_id)
    db.delete(session)
    db.commit()


def dev_send_message(db: Session, org_id: int, message: str, session_id: str | None = None) -> dict:
    ...
    if not session_id:
        session_id = create_session(db, org_id)["id"]
    else:
        _get_owned_session(db, org_id, session_id)

    return post_user_message(db, org_id, session_id, message)


def post_user_message(db: Session, org_id: int, session_id: str, message: str) -> dict:
    session = _get_owned_session(db, org_id, session_id)
    ...
    history = (
        ...
        .filter(ChatMessage.session_id == session.id, ChatMessage.org_id == org_id)
    ...
    ctx = ToolContext(db=db, org_id=org_id)
